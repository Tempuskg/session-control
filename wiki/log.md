...
## [2026-08-02] update | Listing Overview Restructure (Step 3)
Restructured README.md so the Open VSX and VS Marketplace overview reads as a listing page rather than an engineering spec. Moved the deep auto-save reference (per-source contracts, project matching, profiles/Remote SSH/dev containers/WSL boundaries, diagnostics report fields) verbatim into the new `docs/auto-save.md` and linked it; added a Quick start section and a Commands table; replaced the hardcoded `v1.3.6` VS Marketplace badge with dynamic version/download badges; consolidated Requirements and Installation below the pitch. README dropped from 419 to 293 lines with no factual claim removed. `docs/**` added to .vscodeignore — vsce rewrites the relative link to an absolute GitHub URL at package time, so the files need not ship in the VSIX.
Pages touched: README.md, docs/auto-save.md, .vscodeignore, open-vsx-listing.md, log.md
