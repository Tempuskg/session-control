...
function createSession(id, savedAt, title, content, requestPrompt = `Prompt for ${title}`) {
    return {
        version: 1,
        id,
        title,
...
function createCandidate(id, savedAt, title, content, requestPrompt) {
    return {
...
suite('sessionAnalysis', () => {
    test('parseAnalysisSelectionAlias recognizes presets and needs-analysis aliases', () => {
        ...
        assert.equal((0, sessionAnalysis_1.parseAnalysisSelectionAlias)('24h', now)?.mode, 'last24Hours');
        assert.equal((0, sessionAnalysis_1.parseAnalysisSelectionAlias)('7d', now)?.mode, 'last7Days');
        assert.equal((0, sessionAnalysis_1.parseAnalysisSelectionAlias)('30d', now)?.mode, 'last30Days');
        assert.equal((0, sessionAnalysis_1.parseAnalysisSelectionAlias)('needs analysis', now)?.mode, 'needsAnalysis');
    });
    test('createCustomRangeSelection rejects inverted ranges', () => {
        assert.throws(() => (0, sessionAnalysis_1.createCustomRangeSelection)('2026-05-20T00:00:00.000Z', '2026-05-10T00:00:00.000Z'), /start must be before the end/i);
    });
    test('filterCandidatesForAnalysis filters by timeframe and unanalyzed state', () => {
        ...
        const last24 = (0, sessionAnalysis_1.createPresetAnalysisSelection)('last24Hours', now);
        const byTime = (0, sessionAnalysis_1.filterCandidatesForAnalysis)([recent, older], last24, new Set());
        assert.equal(byTime.length, 1);
        assert.equal(byTime[0]?.session.id, 'recent');
        const needsAnalysis = (0, sessionAnalysis_1.createNeedsAnalysisSelection)();
        const unanalyzed = (0, sessionAnalysis_1.filterCandidatesForAnalysis)([recent, older], needsAnalysis, new Set(['fingerprint-recent']));
        assert.equal(unanalyzed.length, 1);
        assert.equal(unanalyzed[0]?.session.id, 'older');
        const last24UnanalyzedOnly = (0, sessionAnalysis_1.createPresetAnalysisSelection)('last24Hours', now, true);
        const withinRangeUnanalyzedOnly = (0, sessionAnalysis_1.filterCandidatesForAnalysis)([recent, older], last24UnanalyzedOnly, new Set(['fingerprint-recent']));
        assert.equal(withinRangeUnanalyzedOnly.length, 0);
        const customRangeUnanalyzedOnly = (0, sessionAnalysis_1.createCustomRangeSelection)('2026-05-01T00:00:00.000Z', '2026-05-18T00:00:00.000Z', true);
        const mixedRange = (0, sessionAnalysis_1.filterCandidatesForAnalysis)([recent, older], customRangeUnanalyzedOnly, new Set(['fingerprint-recent']));
    ...
    test('filterCandidatesForAnalysis skips saved session-control analyze chats', () => {
        ...
        const analyze = createCandidate('analyze', '2026-05-17T10:00:00.000Z', '@session-control /analyze', 'Analyze content', '@session-control /analyze');
        const analyzeTypo = createCandidate('analyze-typo', '2026-05-17T09:00:00.000Z', '@session-control /analze', 'Analyze typo content', '@session-control /analze');
        const filtered = (0, sessionAnalysis_1.filterCandidatesForAnalysis)([normal, analyze, analyzeTypo], (0, sessionAnalysis_1.createNeedsAnalysisSelection)(), new Set());
    ...
    test('splitCandidatesIntoAnalysisBatches splits large candidate sets by evidence size', () => {
        ...
        const batches = (0, sessionAnalysis_1.splitCandidatesIntoAnalysisBatches)([first, second], 250);
        assert.equal(batches.length, 2);
        assert.equal(batches[0]?.length, 1);
        assert.equal(batches[1]?.length, 1);
    });
    test('buildAnalysisPrompt condenses oversized evidence for compact detail levels', () => {
        const selection = (0, sessionAnalysis_1.createNeedsAnalysisSelection)();
        ...
        const full = (0, sessionAnalysis_1.buildAnalysisPrompt)(selection, [candidate]);
        const compact = (0, sessionAnalysis_1.buildAnalysisPrompt)(selection, [candidate], '', 'compact');
        const summaryOnly = (0, sessionAnalysis_1.buildAnalysisPrompt)(selection, [candidate], '', 'summaryOnly');
    ...
    test('buildAnalysisPrompt includes timeframe label and required sections', () => {
        const selection = (0, sessionAnalysis_1.createPresetAnalysisSelection)('last7Days', new Date('2026-05-17T12:00:00.000Z'));
        const prompt = (0, sessionAnalysis_1.buildAnalysisPrompt)(selection, [
    ...
    test('buildAnalysisSynthesisPrompt includes batch findings for final synthesis', () => {
        const selection = (0, sessionAnalysis_1.createNeedsAnalysisSelection)();
        const prompt = (0, sessionAnalysis_1.buildAnalysisSynthesisPrompt)(selection, ['Finding one', 'Finding two'], '### Workspace: workspace\n\n#### SKILL.md\n\n```md\nExisting skill\n```');
    ...
    test('buildImplementationHandoffPrompt points the coding agent at the saved report file', () => {
        const prompt = (0, sessionAnalysis_1.buildImplementationHandoffPrompt)('e:/workspace/.chat/analysis/reports/report-1.md', 'Implement the highest-priority recommendation.');
...
//# sourceMappingURL=sessionAnalysis.test.js.map