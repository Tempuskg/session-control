...
async function main() {
    try {
        ...
        const downloadedExecutablePath = testVersion === undefined
            ? await (0, test_electron_1.downloadAndUnzipVSCode)()
            : await (0, test_electron_1.downloadAndUnzipVSCode)({ version: testVersion });
        const vscodeExecutablePath = process.platform === 'win32'
            ? path.join(path.dirname(downloadedExecutablePath), 'bin', testVersion === 'insiders' ? 'code-insiders.cmd' : 'code.cmd')
